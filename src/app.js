...
app.get('/teams/:id', async (req, res) => {
    try {
        ...
        //Devuelvo el error
        res.status(404).json({message: "Error", errorMessage: "Team not found."})
    } catch (error) {
        console.log(error)
...
app.post('/teams', async (req, res) => {
    try {
        ...
        await fs.writeFile(__dirname+'/db/database.txt', stringTeamsParsed ,'utf-8');
